...
if "${HL_COMPOSER_CLI}" card list -n PeerAdmin@oslab > /dev/null; then
    "${HL_COMPOSER_CLI}" card delete -n PeerAdmin@oslab
fi
"${HL_COMPOSER_CLI}" card create -p ./card/connection.json -u PeerAdmin -c "${CERT}" -k "${PRIVATE_KEY}" -r PeerAdmin -r ChannelAdmin --file ./card/PeerAdmin@oslab.card
"${HL_COMPOSER_CLI}" card import --file ./card/PeerAdmin@oslab.card 

echo "Hyperledger Composer PeerAdmin card has been imported"
"${HL_COMPOSER_CLI}" card list

composer network install -c PeerAdmin@oslab -a oslab/oslab@0.0.1.bna

composer network start --networkName oslab --networkVersion 0.0.1 --networkAdmin admin --networkAdminEnrollSecret adminpw --card PeerAdmin@oslab 

composer card import --file admin@oslab.card 

composer-rest-server -c admin@oslab -n always -u true -w true -p 5000
